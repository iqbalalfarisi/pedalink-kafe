    ...
    <div class="box box-success">
        <form method="POST">
            <div class="box-header with-border">
                <h3 class="box-title">KERANJANG PESANAN RESERVASI</h3>
                <br><br>
                ...
                <div class="form-group">
                    <select name="cbpesanan" class="form-control select2" style="width: 100%;">
                        <?php
                        $qry = mysqli_query($konek, "select `header_pesanan`.`kode` AS `kode`,`header_pesanan`.`id_pengguna` AS `id_pengguna`,`header_pesanan`.`tanggal` AS `tanggal`,`header_pesanan`.`total` AS `total`,`header_pesanan`.`kode_reservasi` AS `kode_reservasi`,`header_reservasi`.`nama_pelanggan` AS `nama_pelanggan`,`header_pesanan`.`status` AS `status` from (`header_pesanan` join `header_reservasi` on((`header_reservasi`.`kode` = `header_pesanan`.`kode_reservasi`))) where (`header_pesanan`.`status` = 'Proses')");
                        while ($d = mysqli_fetch_array($qry)) { ?>
                        <option class="form-control" value="<?php echo $d["kode"]; ?>"><?php echo $d['nama_pelanggan']; ?>
                        </option>
                        <?php } ?>
                    </select>
                </div>
                ...
                <a href="beranda.php?page=pilih_menu" class="btn btn-warning fa fa-shopping-cart"> Tambah Order</a>
            </div>
                ...
                <table class="table table-bordered table-striped">
                    <thead>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="beranda.php?page=card_2&index=<?php echo $index; ?>" class="btn btn-danger fa fa-close"> Hapus</a>
                            </td>
                        </tr>
                        <?php
                            $index++;
        ...
        if (isset($_POST["btnsimpan"])) {
            ...
            for ($i = 0; $i < count($cart); $i++) {
                ...
                $sql2 = 'INSERT INTO detail_pesanan (id_menu, id_pesanan, harga, jumlah,status) VALUES (' . $cart[$i]->kode . ', ' . $cbpesanan . ', ' . $cart[$i]->harga . ', ' . $cart[$i]->quantity . ', ' . $cart[$i]->dekker . ')';
        ...
        <?php
                ...
                echo "<meta http-equiv='refresh' content='0; url=?page=card_2'>";
            }
